...
Positionable PositionableHandler::createItem(std::string type, int id) {
  ...
  else if (type == "machine_gun") return Gun(type, id, cp.getSpecificGun(type)[0], cp.getSpecificGun(type)[1], cp.getSpecificGun(type)[2]);
  else if (type == "chain_gun") return Gun(type, id, cp.getSpecificGun(type)[0], cp.getSpecificGun(type)[1], cp.getSpecificGun(type)[2]);
  else if (type == "rpg_gun") return Gun(type, id, cp.getSpecificGun(type)[0], cp.getSpecificGun(type)[1], cp.getSpecificGun(type)[2]);
